...
mysql_select_db("homevisit") or die(mysql_error());
    $id_app = $_GET['id_app'];
//    echo $id_app;
    $editval = "SELECT * FROM calendar_info WHERE Id_app=$id_app";
    $link = mysqli_connect("localhost", "hvmsdb", "1234", "homevisit");
    $calen_info = mysqli_query($link,$editval) or die(mysqli_error()."[".$editval."]");
    $calen_info_array = mysqli_fetch_array($calen_info);
?>
                ...
                <main class="mdl-layout__content mdl-color--grey-100">
                    <div class="mdl-grid demo-content">
                        ...
                        <div id="content" class="mdl-shadow--2dp">
                            ...
                            <form class="uk-form-horizontal" action="calendar_edit_db.php" method="get">
                                <div class="uk-margin">
                                    ...
                                    <div class="uk-form-controls">
                                        ...
                                        {
                                            if ($calen_info_array['patient_hn'] === $row['patient_hn']) echo "<option value='".$row['patient_hn']."' selected=\"selected\">".$row['patient_p_name'].$row['patient_name']." ".$row['patient_surname']." (HN ".$row['patient_hn'].")"."</option>";
                                ...
                                <div class="uk-margin">
                                    ...
                                    <div class="uk-form-controls">
                                        <input class="uk-input uk-width-medium" type="date" name="datepicker" value="<?php echo $calen_info_array['dmy'];?>"> </div>
                                </div>
                                <div class="uk-margin">
                                    <div class="uk-form-label">ช่วงเวลา</div>
                                    <div class="uk-form-controls uk-form-controls-text">
                                        <label>
                                            <input class="uk-radio" type="radio" name="visit-time" value="0" <?php if ($calen_info_array[ 'time_calen']==0) echo "checked";?>> เช้า (9.00-12.00 น)</label>
                                        <div class="uk-margin-small">
                                            <label>
                                                <input class="uk-radio " type="radio" name="visit-time" value="1" <?php if ($calen_info_array[ 'time_calen']==0) echo "checked";?>> บ่าย (13.00-16.00 น)</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="uk-margin">
    ...
    mysqli_query($link,"SET character_set_connection=utf8");
                                        ...
                                        while ($sleep = mysqli_fetch_assoc($quedoc))
                                        {
                                            while ($ro = mysqli_fetch_assoc($r))
                                                {
//                                                echo $sleep['Id_members'];
//                                                echo $sleep['Id_members']."||";
                                                    if ($sleep['Id_members'] == $ro['user'] ) echo "<option value='".$ro['user']."' selected=\"selected\">".$ro['f_user']." ".$ro['l_user']." (".$ro['user'].")"."</option>";
                                    ...
                                    ?>
                                        ...
                                        <input name="id_app" value="<?php echo $id_app;?>" style="visibility:hidden">
                                    </div>
                                </div>
